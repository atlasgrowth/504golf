...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Add endpoint for serving an order - sets every item status to DELIVERED
  app.post('/api/order/:id/serve', async (req: Request, res: Response) => {
    try {
      const orderId = req.params.id;
      ...
      res.status(500).json({ message: 'Failed to serve order' });
    }
  });
